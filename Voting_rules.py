...
# Here I'm defining the dictatorship function. An agent is selected, and the winner is the alternative that this agent ranks first #
...
# Here I'm defining the testscoringRule function and the function contains error-handling code for the case when the length of the scoring vector is not m. In that case, the message "Incorrect input" will be printed and the function will return False #
...
# Here I'm defining the plurality and the winner is the alternative that appears the most times in the first position of the agents preference orderings #
...
# Here I'm defining the plurality and Every agent assigns 0 points to the alternative that they rank in the last place of their preference orderings, and 1 point to every other alternative. The winner is the alternative with the most number of points #
...
# Here I'm defining the baroda Every agent assigns a score of 0 to the their least-preferred alternative (the one at the bottom of the preference ranking), a score of 1 to the second least-preferred alternative, ... , and a score of m-1 to their favourite alternative. In other words, the alternative ranked at position j receives a score of m-j. The winner is the alternative with the highest score #
...
# Here I'm defining the harmonic and Every agent assigns a score of 1/m to the their least-preferred alternative (the one at the bottom of the preference ranking), a score of 1/(m-1) to the second least-preferred alternative, ... , and a score of 1 to their favourite alternative. In other words, the alternative ranked at position j receives a score of 1/j #
...
# Here I'm defining the STV The voting rule works in rounds. In each round, the alternatives that appear the least frequently in the first position of agents' rankings are removed, and the process is repeated. When the final set of alternatives is removed (one or possibly more), then this last set is the set of possible winners #
...
# Here I'm defining rangeVoting function should input a worksheet corresponding to an xlxs file and it should return the alternative that has the maximun sum of valuations and uses tie breaking option to distinguish between possible winners #
...
# Here I'm  defining get max value function and among the possible winning alternatives, it selects the one with the highest number #
...
# Here I'm defining the tie break function and it  will consider the following three tie-breaking rules and assume that the alternatives are represented by integers #
